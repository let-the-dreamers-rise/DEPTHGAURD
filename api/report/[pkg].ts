...
export default async function handler(
  ...
  res: {
    status: (n: number) => { json: (b: unknown) => void; send: (b: string) => void; end: () => void }
  ...
  const q = req.query ?? {}
  const packageName = String(q.pkg ?? q.package ?? '').trim()
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="depthguard-${packageName}-report.md"`)
